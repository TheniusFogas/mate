...
const difficultyColors: Record<QuizDifficulty, string> = { easy: "text-success", medium: "text-warning", hard: "text-destructive" };
const TIMER_SECONDS: Record<QuizDifficulty, number> = { easy: 30, medium: 20, hard: 15 };
...
const QuizMode = ({ onClose }: { onClose: () => void }) => {
  ...
  useEffect(() => {
    if (started && !finished && !showResult && timeLeft === 0 && timerRef.current === null && questions.length > 0) {
      // Timer expired — mark as wrong
      const q = questions[currentIdx];
      if (q && q.isCorrect === null) {
        const updated = [...questions];
        updated[currentIdx] = { ...q, userAnswer: "(timp expirat)", isCorrect: false };
        setQuestions(updated);
        setShowResult(true);
      }
    }
  }, [timeLeft, started, finished, showResult]);
  ...
  const handleSubmit = () => {
    ...
    const updated = [...questions];
    const timeUsed = TIMER_SECONDS[difficulty] - timeLeft;
  ...
  if (showLeaderboard) {
    const shareText = scores.length > 0
      ? `🏆 Clasament MathLab Quiz:\n${scores.slice(0, 5).map((s, i) => `${i+1}. ${s.name} — ${s.score}/${s.total} (${difficultyLabels[s.difficulty]})`).join("\n")}\n\n${window.location.origin}`
      : "";
    return (
      <Overlay>
        <Panel>
          <Header title="🏆 Clasament" onClose={() => setShowLeaderboard(false)}
            extra={scores.length > 0 ? (
              <button onClick={() => navigator.clipboard.writeText(shareText)}
                className="text-muted-foreground hover:text-foreground" title="Partajează clasament">
            ...
            {scores.length === 0 ? (
              ...
              <>
                ...
                <div className="flex items-center justify-center gap-2 pt-2 border-t border-border/30">
                  <span className="text-[9px] text-muted-foreground">Partajează:</span>
                  ...
                  <button onClick={() => navigator.clipboard.writeText(shareText)}
                    className="h-6 w-6 rounded-full bg-accent flex items-center justify-center text-accent-foreground hover:opacity-80 transition-opacity" title="Copiază">
        ...
        <div className="p-4">
          {!started ? (
            ...
            <QuestionView current={current} currentIdx={currentIdx} total={questions.length}
              ...
              showResult={showResult} difficulty={difficulty} timeLeft={timeLeft} maxTime={TIMER_SECONDS[difficulty]}
        ...
        {(["easy", "medium", "hard"] as QuizDifficulty[]).map(d => (
          <button key={d} onClick={() => setDifficulty(d)}
            ...
            <span className="block text-[8px] opacity-70">
              {d === "easy" ? `3 variante • ${TIMER_SECONDS[d]}s` : d === "medium" ? `6 variante • ${TIMER_SECONDS[d]}s` : `Liber • ${TIMER_SECONDS[d]}s`}
            </span>
          </button>
        ))}
      </div>
...
const FinishedView = ({ score, questions, difficulty, totalTime, playerName, setPlayerName, scoreSaved, onSaveScore, onRestart, onClose }: {
  ...
  const text = `Am obținut ${score}/${questions.length} la Quiz-ul Matematic (${difficultyLabels[difficulty]}) în ${totalTime}s! 🧠🎯`;
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-3">
      ...
      <div className="flex items-center justify-center gap-2 text-[10px]">
        <span className={`font-medium ${difficultyColors[difficulty]}`}>{difficultyLabels[difficulty]}</span>
...
const QuestionView = ({ current, currentIdx, total, score, answer, setAnswer, showResult, difficulty, timeLeft, maxTime, onSubmit, onNext, onChoiceSelect }: {
  current: QuizQuestion; currentIdx: number; total: number; score: number;
  answer: string; setAnswer: (s: string) => void; showResult: boolean;
  difficulty: QuizDifficulty; timeLeft: number; maxTime: number;
  onSubmit: () => void; onNext: () => void; onChoiceSelect: (c: string) => void;
}) => {
  const timerPct = (timeLeft / maxTime) * 100;
  const timerColor = timeLeft <= 5 ? "bg-destructive" : timeLeft <= 10 ? "bg-warning" : "bg-primary";
      ...
      <motion.div key={currentIdx} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-3">
        ...
        {showResult && (
          <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
            ...
            {current.isCorrect
              ? <><Check className="h-3.5 w-3.5 text-primary" /><span className="text-primary font-medium">Corect! 🎉</span></>
