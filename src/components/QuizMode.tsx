...
import { Brain, ChevronRight, Check, X, RotateCcw, Trophy, Share2, Crown, Trash2 } from "lucide-react";
...
import { getScores, addScore, clearScores, type ScoreEntry } from "@/lib/quizScores";

const difficultyLabels: Record<QuizDifficulty, string> = { easy: "Ușor", medium: "Mediu", hard: "Greu" };
const difficultyColors: Record<QuizDifficulty, string> = { easy: "text-green-400", medium: "text-yellow-400", hard: "text-red-400" };
...
const QuizMode = ({ onClose }: { onClose: () => void }) => {
  ...
  const handleStart = () => {
    setQuestions(generateQuiz(questionCount, categoryId, difficulty));
    setCurrentIdx(0);
    setAnswer("");
    setShowResult(false);
    setFinished(false);
  ...
  const handleSaveScore = () => {
    ...
    const catName = categoryId ? mathCategories.find(c => c.id === categoryId)?.name || "Toate" : "Toate";
  ...
  if (showLeaderboard) {
    return (
      <Overlay>
        <Panel>
          <Header title="🏆 Clasament" onClose={() => setShowLeaderboard(false)} />
          <div className="p-4 space-y-2">
            {scores.length === 0 ? (
              <p className="text-center text-[11px] text-muted-foreground py-6">Niciun scor salvat încă.</p>
            ) : (
              <>
                {scores.map((s, i) => (
                  <div key={s.id} className={`flex items-center gap-2 rounded-[5px] p-2 text-[10px] ${i === 0 ? "bg-primary/10 border border-primary/20" : "bg-secondary/30"}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="text-[8px] text-muted-foreground">{s.category} • <span className={difficultyColors[s.difficulty]}>{difficultyLabels[s.difficulty]}</span></span>
                    </div>
                    <span className="font-mono font-bold text-primary">{s.score}/{s.total}</span>
                  </div>
                ))}
                <button onClick={() => { clearScores(); setShowLeaderboard(false); }} className="w-full mt-2 py-1 rounded-[5px] text-[9px] text-destructive hover:bg-destructive/10 flex items-center justify-center gap-1">
                  <Trash2 className="h-3 w-3" /> Șterge clasament
                </button>
              </>
            )}
          </div>
  ...
  return (
    <Overlay>
      <Panel>
        <Header title="Quiz Matematic" icon={<Brain className="h-4 w-4 text-primary" />} onClose={onClose}
          extra={<button onClick={() => setShowLeaderboard(true)} className="text-muted-foreground hover:text-foreground" title="Clasament"><Crown className="h-4 w-4" /></button>}
        />
        <div className="p-4">
          {!started ? (
            <SetupView
            ...
            <FinishedView
              score={score} questions={questions} difficulty={difficulty}
            ...
            <QuestionView
              current={current} currentIdx={currentIdx} total={questions.length}
              score={score} answer={answer} setAnswer={setAnswer}
              showResult={showResult} difficulty={difficulty}
...
const SetupView = ({ categoryId, setCategoryId, questionCount, setQuestionCount, difficulty, setDifficulty, onStart }: {
  ...
  <div className="space-y-3">
    <div>
      <label className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider block mb-1">Dificultate</label>
      <div className="flex gap-1">
        {(["easy", "medium", "hard"] as QuizDifficulty[]).map(d => (
          <button key={d} onClick={() => setDifficulty(d)}
            ...
            <span className="block font-semibold">{difficultyLabels[d]}</span>
            <span className="block text-[8px] opacity-70">{d === "easy" ? "3 variante" : d === "medium" ? "6 variante" : "Răspuns liber"}</span>
          </button>
        ))}
      </div>
    </div>
...
const FinishedView = ({ score, questions, difficulty, playerName, setPlayerName, scoreSaved, onSaveScore, onRestart, onClose }: {
  score: number; questions: QuizQuestion[]; difficulty: QuizDifficulty;
  playerName: string; setPlayerName: (s: string) => void;
  scoreSaved: boolean; onSaveScore: () => void;
  onRestart: () => void; onClose: () => void;
}) => {
  const text = `Am obținut ${score}/${questions.length} la Quiz-ul Matematic (${difficultyLabels[difficulty]})! 🧠🎯`;
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-3">
      ...
      <div className="flex items-center justify-center gap-1">
        <span className={`text-[10px] font-medium ${difficultyColors[difficulty]}`}>
      ...
      {!scoreSaved ? (
        <div className="flex gap-1">
          <input type="text" value={playerName} onChange={e => setPlayerName(e.target.value)} placeholder="Numele tău..."
        ...
        <p className="text-[10px] text-primary font-medium">✅ Scor salvat în clasament!</p>
      )}
      ...
      <div className="flex items-center justify-center gap-2">
        ...
        <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(url)}`} target="_blank" rel="noopener noreferrer"
          className="h-7 w-7 rounded-full bg-foreground flex items-center justify-center text-background hover:opacity-80 transition-opacity" title="X (Twitter)">
          <svg className="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
        </a>
        <button onClick={() => { navigator.clipboard.writeText(`${text}\n${url}`); }}
          className="h-7 w-7 rounded-full bg-accent flex items-center justify-center text-accent-foreground hover:opacity-80 transition-opacity" title="Copiază">
        ...
        {questions.map((q, i) => (
          <div key={i} className={`rounded-[4px] p-1.5 text-[9px] flex items-start gap-1.5 ${q.isCorrect ? "bg-primary/10" : "bg-destructive/10"}`}>
            ...
            <div className="min-w-0">
              ...
              {!q.isCorrect && <span className="text-muted-foreground"> — Corect: {q.correctResults[0]?.value}</span>}
            </div>
          </div>
        ))}
      </div>
...
const QuestionView = ({ current, currentIdx, total, score, answer, setAnswer, showResult, difficulty, onSubmit, onNext, onChoiceSelect }: {
  ...
  <AnimatePresence mode="wait">
    <motion.div key={currentIdx} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-3">
      ...
      <div className="flex items-center justify-between text-[9px] text-muted-foreground">
        <span>Întrebarea {currentIdx + 1}/{total}</span>
        <span className="flex items-center gap-1.5">
          <span className={`${difficultyColors[difficulty]} font-medium`}>{difficultyLabels[difficulty]}</span>
          <span>{score} corecte</span>
        </span>
      </div>
      <div className="h-1 rounded-full bg-secondary overflow-hidden">
        <motion.div className="h-full bg-primary rounded-full" initial={{ width: 0 }}
          animate={{ width: `${((currentIdx + (showResult ? 1 : 0)) / total) * 100}%` }} />
      </div>
            ...
            return (
              <button key={i} onClick={() => onChoiceSelect(choice)} disabled={showResult}
                className={`px-3 py-2 rounded-[5px] text-[11px] font-mono font-medium border transition-all text-left ${
                  isCorrectChoice ? "border-green-500/50 bg-green-500/10 text-green-400" :
      ...
      {showResult && (
        <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
          className={`rounded-[5px] p-2 text-[10px] flex items-center gap-2 ${current.isCorrect ? "bg-green-500/10 border border-green-500/30" : "bg-destructive/10 border border-destructive/30"}`}>
          {current.isCorrect
      ...
      {difficulty === "hard" && !showResult ? (
        ...
        <button onClick={onNext}
          ...
          {currentIdx + 1 >= total ? "Vezi rezultatele" : "Următoarea"} <ChevronRight className="h-3 w-3" />
        </button>
      ) : null}
    </motion.div>
  </AnimatePresence>
